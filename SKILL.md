...
description: 覆盖A股/港股/美股的多源数据量化选股引擎，已实现17种选股策略（S01红肥绿瘦、S02上涨波段、S03回调缩量、S04横盘调整、S05 MACD底背离、S06高股息、S07低估值、S08放量突破、S09趋势分析、S10布林带下轨、S11筹码集中、S12现金流质量、S13 ROE杜邦筛选、S14费雪成长股、S15长期低位蓄力、S16海龟交易、S17动量策略），支持单策略、多策略组合筛选（union/intersection/weighted三种模式）。v1.0.1主流程支持A股/港股/美股三市场执行筛选（港股/美股技术面与估值策略可用，财务摘要/资金流策略如实不通过）。内置交互式新用户引导向导（真实API数据驱动）。触发词（精准触发，覆盖明确选股意图）：按策略名：ROE选股、ROE筛选、MACD选股、MACD筛选、MACD底背离、股息选股、高股息选股、估值选股、低估值、成长股筛选、费雪成长股、低位放量选股、长期低位选股、近期放量、趋势选股、形态选股、K线形态筛选、布林带选股、筹码集中选股、现金流质量、海龟策略、海龟选股、唐奇安通道、动量策略、动量选股、趋势跟踪选股按组合意图：组合选股、筛选股票、多策略选股、综合选股、并发选股、全部策略选股按结果要求：按ROE排名、按评分排名、按股息率排名、取交集、取并集按引导相关：选股引导、新手引导、选股向导、筛选向导明确排除（这些场景应激活其他skill）：- 任何包含具体股票代码/名称的个股分析请求- "帮我看看XX股票"、"XX公司怎么样"、"XX值不值得买"- "查一下行情"、"看资金流向"等纯数据查询
---

# 多策略量化筛选 Skill（stock-selecter-pro）v1.0.1
...
- **市场能力边界如实**：A股全能力；港股/美股支持技术面与估值策略（S06/S07），依赖财务摘要/资金流的策略（S12-S15）因无公开数据如实判定不通过。
...
│   │   ├── money_flow.py       # 主力资金/北向资金/连续流入
│   │   ├── sector.py           # 行业板块/概念板块排行及成分股
│   │   ├── hk_share.py         # 港股行情/列表/K线（主板+创业板）
│   │   └── us_share.py         # 美股行情/列表/K线（NYSE+NASDAQ）
│   ├── indicators/             # 技术指标库
│   │   ├── __init__.py         # 导出 MA/EMA/MACD/KDJ/RSI/BOLL/ADX/ATR/OBV
│   │   └── technical.py        # 标准技术指标（MACD/KDJ/RSI/BOLL/均线/ADX/ATR/OBV）
...
│   ├── cache/                  # 缓存层
│   │   └── __init__.py         # KlineCacheManager + FundamentalCacheManager
...
| S06 | 高股息策略 | `strategies/s03_high_dividend.py` | `check_s03_high_dividend` | 日K线 + 估值数据 |
| S07 | 低估值策略 | `strategies/s04_low_valuation.py` | `check_s04_low_valuation` | 日K线 + 估值数据 |
| S08 | 放量突破 | `strategies/s05_volume_breakout.py` | `check_s05_volume_breakout` | 日K线 |
| S09 | 趋势分析 | `strategies/s06_trend_analysis.py` | `check_s06_trend_analysis` | 日K线 |
| S10 | 布林带下轨反弹 | `strategies/s07_bollinger.py` | `check_s07_bollinger` | 日K线 |
| S11 | 筹码集中度 | `strategies/s08_chip_concentration.py` | `check_s08_chip_concentration` | 日K线 |
| S12 | 现金流质量 | `strategies/s09_cashflow_quality.py` | `check_s09_cashflow_quality` | 日K线 + 财务数据 |

### 进阶策略 (S13-S15)

| 编号 | 策略 | 策略文件 | 判定函数 | 依赖数据 | 创新点 |
|------|------|---------|---------|---------|--------|
| S13 | ROE杜邦筛选 | `strategies/s10_roe_screening.py` | `check_s10_roe_screening` | 日K线 + 财务数据 | ROE+盈利质量+杠杆健康 |
| S14 | 费雪成长股 | `strategies/s11_fisher_growth.py` | `check_s11_fisher_growth` | 日K线 + 财务数据 | PEG+成长性确认 |
| S15 | 长期低位蓄力 | `strategies/s12_long_consolidation.py` | `check_s12_long_consolidation` | 日K线 + 资金流 | 深度回调+底部放量 |

### 趋势跟踪/动量策略 (S16-S17)

| 编号 | 策略 | 策略文件 | 判定函数 | 依赖数据 | 算法核心 |
|------|------|---------|---------|---------|---------|
| S16 | 海龟交易 | `strategies/s13_turtle.py` | `check_s13_turtle` | 日K线 | 唐奇安通道突破（收盘突破近20日最高）+ ADX趋势过滤 + MA60顺势 + 2*ATR止损参考 |
| S17 | 动量策略 | `strategies/s14_momentum.py` | `check_s14_momentum` | 日K线 | 近60日动量≥8% + 近20日动能延续 + MA60趋势背景 + RSI/乖离过热过滤 |
...
- **A股**：`data/a_share.py` — 全市场列表、日/周/月/60分钟K线、批量并发获取、实时快照
...
- **资金流**：`data/money_flow.py` — 个股主力资金、北向资金、连续N日净流入判断
- **板块**：`data/sector.py` — 申万一级行业排行、概念板块排行、板块成分股

数据源：东方财富公开 HTTP API（`push2.eastmoney.com` / `push2his.eastmoney.com` / `datacenter.eastmoney.com`）为主通道，腾讯公开K线（`proxy.finance.qq.com`）与新浪美股（`stock.finance.sina.com.cn`）为容灾备选，请求间隔自动控制防封。

**网络健壮性（真实故障切换）**：
- 实时接口（列表/行情/估值）经 `data/_http.py` 共享客户端请求，主节点不可达时自动切换官方备用延迟节点 `push2delay.eastmoney.com`（延迟约3分钟，字段一致）。
- 历史K线接口（push2his）自动尝试多编号节点（79/92/91/无编号），首个返回有效数据的节点即用；全部节点不可达时自动切换备选公开通道：A股/港股走腾讯公开K线（proxy.finance.qq.com 优先，web.ifzq 触发风控时轮询备用入口），美股走新浪美股全历史日K（腾讯接口对美股仅返回首末两条，实测不可用）。备选通道输出与东财同构的K线字段（缺省字段如实置0，涨跌幅由收盘价序列真实计算），确保回测与技术面策略真实可用。
...
港股/美股标的缺少财务摘要/资金流数据时，S12/S13/S14/S15 将如实判定为不通过（不伪造数值），估值策略 S06/S07 依赖的估值接口三市场均返回真实数据、可用；建议港股/美股仅使用技术面策略 S01-S05、S08-S11、S16-S17（S16 海龟交易 / S17 动量策略均为纯K线技术面）与估值策略 S06/S07。命令行会在执行前给出提示。
...
- **weighted**（加权）：`Σ(策略得分 × 策略权重) / Σ权重 × 10`，权重通过 `--weights "S01=0.15,S05=0.25,..."` 配置
...
python run_screening.py \
  ...
  --mode hs300              \  # hs300|zz500|full（非指数成分近似采样，如实标注）
...
**引导关闭方式**：引导完成并执行筛选后写入标记文件（`~/.stock_selecter_pro_guide_done`），下次运行自动跳过；亦可通过 `--no-guide` 或引导内选择"下次不再显示"。引导中任一环节输入 `q` 退出则**不写标记**，下次仍会引导。
...
| 日K线 | `79.push2his.eastmoney.com/api/qt/stock/kline/get` | 日/周/月/分钟K线（主通道） |
...
| 财务摘要 | `emweb.securities.eastmoney.com/PC_HSF10/FinanceSummary/` | ROE/EPS/毛利率等 |
...
4. **港股/美股数据能力边界**：港股无财务摘要/资金流公开数据（K线与周线可用），美股无财务摘要/资金流/周线公开数据（仅日K线可用）；依赖财务摘要/资金流的策略（S12/S13/S14/S15）在港股/美股如实判定不通过，不伪造数值；估值策略 S06/S07 与纯技术面策略（S01-S05/S08-S11/S16-S17）三市场可用
