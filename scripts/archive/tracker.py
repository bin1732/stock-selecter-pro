"""
stock-selecter-pro v1.0.4 筛选结果追踪器。
...
def _secid_for_code(code: str) -> Optional[str]:
    """推断标的在东方财富K线接口的 secid（三市场）。

    - 美股：字母代码 → 105.（NYSE/NASDAQ）
    - 港股：数字且 ≤5 位 → 116.（主板；不足5位补零）
    - A股：6位数字 → 6/9 开头为 1.沪，其余 0.深/京
    无法识别时返回 None。
    """
    code = code.strip()
    if not code:
...
def _kline_url_params(stock_code: str) -> Optional[dict]:
    """根据股票代码生成东方财富日K线请求参数（三市场 secid 推断）。"""
...
def refresh_performance(db_path: str) -> dict:
    """遍历 pick_performance 中 return_1w / return_1m 为空缺的记录，
    ...
    rows = conn.execute(
        ...
        "WHERE (return_1w IS NULL OR return_1m IS NULL) AND snapshot_date <= ?",
        (cutoff_date,),
    ).fetchall()
    ...
    for row in rows:
        ...
        # 1月口径：快照满28天时回填"截至最新收盘价的累计收益"（非严格持有1月，如实标注）
        return_1m = round((latest_price - snapshot_price) / snapshot_price * 100, 2)

        snapshot_dt = datetime.strptime(snapshot_date, "%Y-%m-%d")
        days_diff = (datetime.now() - snapshot_dt).days

        # 1周收益与1月收益独立回填：
        # - return_1w 未填 → 随时可回填（快照距今 ≥7 天已由查询条件保证）
        # - return_1m 未填且距今 ≥28 天 → 回填（截至最新收盘的累计收益，与1周同口径）
        if row["return_1w"] is None:
            conn.execute(
            ...
            updated_1w += 1
        if days_diff >= 28 and row["return_1m"] is None:
            conn.execute(
                "UPDATE pick_performance SET check_date_1m=?, price_1m=?, return_1m=? "
                "WHERE pick_id=?",
                (check_date, latest_price, return_1m, pick_id),
            )
            updated_1m += 1
