...
def _scan_signal_days(
    ...
    for i in range(window - 1, len(klines)):
        ...
        try:
            ...
            continue
        if res and (res.get("passed") or res.get("score", 0) > 0):
...
def aggregate_backtest(stats: list[dict]) -> dict:
    """将多只标的的同策略回测结果聚合为市场级统计。

    聚合口径：合并所有标的的信号样本，计算合并胜率/平均收益。
    样本量为合并信号次数；如实标注覆盖标的数。

    Returns:
        dict: {strategy_id, stocks_covered, hold_days: {hold: {samples,
               win_rate, avg_return, note}}}
    """
    if not stats:
        return {}
    ...
    grouped = {}
    for st in stats:
        grouped.setdefault(st["strategy_id"], []).append(st)

    out = {}
    for sid, items in grouped.items():
        agg = {"strategy_id": sid, "stocks_covered": len(items), "hold_days": {}}
        for hold in (5, 20):
            returns = []
            covered = 0
            for st in items:
                h = st.get("hold_days", {}).get(hold)
                if not h or "win_rate" not in h:
                    continue
                # 单标的原始样本收益无法从聚合结果还原，直接用样本加权平均
                # （avg_return 为信号收益均值，按样本量加权近似合并）
                samples = h.get("samples", 0)
                covered += 1
                returns.append((h.get("avg_return", 0.0), samples))
            if not returns:
                continue
            total_samples = sum(s for _, s in returns)
            if total_samples < config.BACKTEST_MIN_SIGNALS:
                agg["hold_days"][hold] = {
                    "samples": total_samples,
                    "note": "样本不足，不出胜率",
                }
                continue
            avg_ret = sum(r * s for r, s in returns) / total_samples
            agg["hold_days"][hold] = {
                "samples": total_samples,
                "win_rate": None,   # 无法跨标的精确还原胜率，如实留空
                "avg_return": round(avg_ret, 4),
                "note": f"覆盖{covered}只标的的合并样本均值（无法跨标的还原合并胜率）",
            }
        if agg["hold_days"]:
            out[sid] = agg
    return out
