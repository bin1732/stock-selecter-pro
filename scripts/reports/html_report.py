...
def generate_html_report(
    ...
    Args:
        results: 筛选结果列表（已按 --top 截断，仅含展示名单）。
        output_dir: 输出目录。
        market_env: 大盘环境数据。
        strategy_stats: 策略统计。
    ...
    total = total_passed if total_passed is not None else len(results)
...
<div class="container">
  ...
  <div class="section">
    ...
    <div class="sort-controls">
      ...
      <button onclick="sortTable('score')" data-sort="score" class="active">按评分</button>
      <button onclick="sortTable('consensus')" data-sort="consensus">按共识度</button>
      <button onclick="sortTable('code')" data-sort="code">按代码</button>
    </div>
...
function sortTable(key) {{
  ...
  sortKey = key;
  // 用 data-sort 精确定位按钮，避免 onclick 回调中 this 指向 window 导致排序失效
  document.querySelectorAll('.sort-controls button').forEach(b => b.classList.remove('active'));
  const btn = document.querySelector('.sort-controls button[data-sort="' + key + '"]');
  if (btn) btn.classList.add('active');
  renderTable();
}}

// HTML 转义（服务端已转义，客户端二次转义防止特殊字符破坏页面结构）
function esc(s) {{
...
function renderTable() {{
  ...
  rows.forEach(r => {{
    ...
    html += '<td><strong>' + esc(score) + '</strong></td>';
    html += '<td>' + esc(consensus) + '</td>';
    html += '<td>' + hits.map(s => '<span class="tag tag-pass">' + esc(s) + '</span>').join('') + '</td>';
...
function renderMiniChart(r) {{
  ...
  return '<div class="mini-chart">' + r.recent_pcts.map(v => {{
    ...
    return '<div class="mini-bar ' + cls + '" style="height:' + h + 'px" title="' + esc(v.toFixed(1) + '%') + '"></div>';
  }}).join('') + '</div>';
}}
