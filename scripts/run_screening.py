"""
stock-selecter-pro v1.0.1 选股筛选主入口。
...
用法:
    ...
    python run_screening.py --market A股 --mode hs300 --strategies S01,S03,S05
...
def _build_single_market_pool(market: str, mode: str, cap: int) -> list[dict]:
    ...
    Args:
        market: A股 / 港股 / 美股
        mode: hs300/zz500/full（非指数成分精确匹配，按公开数据的近似排序取前cap）
        cap: 候选池上限
    ...
    if market == config.MARKET_A:
        print(f"  获取A股全市场列表...")
        all_stocks = fetch_all_a_share_codes()
        print(f"  全市场共 {len(all_stocks)} 只标的")

        # 过滤 ST / 退市 / 新股（N/C 仅当名称以该字母开头时视为新股，避免误杀）
        filtered = []
        for s in all_stocks:
            name = s.get("name", "")
            ...
            if name.startswith(("N", "C")):
                continue
            filtered.append(s)
        print(f"  过滤后共 {len(filtered)} 只")

        if mode == "hs300":
            # v1.0.1 简化采样：按代码顺序取前 cap 只。
            # 严格沪深300指数成分列表需接入指数成分API，留待后续版本，此处不冒充真实成分股。
            filtered.sort(key=lambda x: x.get("code", ""))
        filtered = filtered[:cap]

        for s in filtered:
            s["market"] = config.MARKET_A
        print(f"  最终候选池: {len(filtered)} 只 (A股)")
        return filtered
...
def _build_candidate_pool(market: str, mode: str, all_cap: Optional[int] = None) -> list[dict]:
    """构建筛选候选池。

    Args:
        market: A股/港股/美股/全部
        mode: hs300/zz500/full
    ...
    Returns:
        list[dict]: [{code, name, market}, ...]
    """
    cap = config.MODE_MARKET_CAP.get(mode, 1000)
    ...
    if market in (config.MARKET_A, config.MARKET_HK, config.MARKET_US):
        return _build_single_market_pool(market, mode, cap=cap)
...
def _fetch_fundamental_by_market(candidates: list[dict]) -> dict[str, dict]:
    """按市场获取基本面/估值数据。

    - A股：财务摘要 + 估值（emweb + push2 接口）
    ...
    for mkt, codes in by_market.items():
        if mkt == config.MARKET_A:
            print(f"  获取A股基本面+估值数据 ({len(codes)} 只)...")
            result.update(
                fetch_fundamental_batch(codes, max_workers=config.MAX_CONCURRENT_WORKERS)
            )
        else:
            print(f"  获取{mkt}估值数据 ({len(codes)} 只)...")
...
def _fetch_a_share_money_flow(a_codes: list[str]) -> dict[str, list]:
    """获取A股资金流数据（抽样，港股/美股无对应公开数据源）。"""
    money_flow_data = {}
    if not a_codes:
        return money_flow_data
    print(f"  获取A股资金流数据 (抽样)...")
    sample_codes = a_codes[:config.MONEY_FLOW_SAMPLE]
    for code in sample_codes:
        try:
            mf = fetch_stock_money_flow(code)
...
def _warn_strategy_applicability(market: str, strategy_ids: list[str]):
    ...
    真实边界（config 中声明）：
    - 估值类 S06/S07：push2 估值接口对 A股/港股/美股 均返回真实 PE/PB/股息率 → 三市场可用
    - 财务摘要类 S12/S13/S14：emweb 接口仅 A股 → 港股/美股无公开数据，自动不通过
    - 资金流类 S15：仅 A股 → 港股/美股无公开数据，自动不通过
    """
    if market == config.MARKET_A:
        return

    selected = set(strategy_ids)
    lacking = sorted(selected & (config.FINANCIAL_STRATEGIES | config.MONEY_FLOW_STRATEGIES))
    usable_val = sorted(selected & config.VALUATION_STRATEGIES)

    if market == config.MARKET_ALL:
        print("  [提示] 全部市场模式下：港股/美股标的的估值策略(S06/S07)可用（真实估值接口三市场返回），")
        print("         财务摘要(S12/S13/S14)与资金流(S15)无公开数据，对港股/美股标的自动不通过。")
        return

    notes = []
    if usable_val:
        notes.append(f"估值策略 {', '.join(usable_val)} 可用（真实估值接口三市场返回）")
    if lacking:
        notes.append(f"策略 {', '.join(lacking)} 无公开数据，将自动不通过（真实缺数据，不伪造）")
    if notes:
        print(f"  [提示] {market}：" + "；".join(notes))
        if lacking:
            print("         建议港股/美股仅使用技术面策略 S01-S05、S08-S11、S16-S17 与估值策略 S06/S07。")
...
def _run_strategies(
    ...
    strategy_hits = [
        ...
        if res.get("passed") or res.get("score", 0) > 0
    ]
...
def run_pipeline(
    ...
    # ------ 2. 候选池 ------
    print(f"\n[2/5] 构建候选股票池...")
    candidates = _build_candidate_pool(market, mode, all_cap=all_cap)
    if not candidates:
        print("  错误: 候选池为空")
        return {
    ...
    fundamental_data = _fetch_fundamental_by_market(valid_candidates)
    a_codes = [c for c in valid_codes if code_market_map.get(c) == config.MARKET_A]
    money_flow_data = _fetch_a_share_money_flow(a_codes)
    ...
    # 合并行业信息：用A股行业/概念排行中的领涨股代码建立 code→行业 映射，
    # 填充入选A股标的的行业字段（港股/美股无对应排行数据，保持"未知"，如实标注）。
    print(f"  获取板块排行数据...")
    try:
    ...
    for r in final_results:
        code = r.get("code")
        if code and industry_map.get(code):
            r["industry"] = industry_map[code]
        elif code and concept_map.get(code):
            r["industry"] = concept_map[code]

    # 趋势推演（真实数据驱动：指数信号 + 板块实时涨幅排行，条件式合规措辞）
    outlook = _build_trend_outlook(market, market_env, industry_ranking, concept_ranking)
    ...
    if output_format in ("text", "all"):
        text_path = generate_text_report(final_results, output_dir, market_env, strategy_stats)
        report_files.append(("Text", text_path))

    # JSON 报告
    if output_format in ("json", "all"):
        json_path = generate_json_report(final_results, output_dir, market_env, strategy_stats)
        report_files.append(("JSON", json_path))

    # HTML 报告
    if output_format in ("html", "all"):
        html_path = generate_html_report(
            final_results, output_dir, market_env, strategy_stats, backtest_data
        )
...
示例:
  ...
  python run_screening.py --multi-period --no-guide
    ...
    parser.add_argument("--mode", default=config.SCREEN_MODE,
                        choices=["hs300", "zz500", "full"],
                        help="筛选范围 (默认: %(default)s)")
    ...
    if should_guide:
        ...
        try:
            ...
            # 仅当用户走完引导并执行筛选（返回结果）时才写"下次跳过"标记；
            # 中途 q 退出 / 取消返回 None，不写标记，下次仍会进入引导。
...
def _run_archive_report(output_dir: str, fmt: str = "all"):
    ...
    if fmt in ("json", "all"):
        ...
        if ranking:
            print(f"TOP3 策略（按1月胜率）:")
            for item in ranking[:3]:
                wr = f"{item['metric_value']*100:.1f}%" if item['metric_value'] else "N/A"
                print(f"  {item['strategy_id']}: {wr}")
...
def run_market_backtest(
    ...
    fund_snapshot = _fetch_fundamental_by_market(valid_bt)
