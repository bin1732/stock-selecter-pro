...
def check_s08_volume_breakout(
    ...
    # ── 条件二：当日涨幅达标 ──
    # 防除零：昨收缺失或为 0 时如实置 0（涨幅无意义，不达标），避免 ZeroDivisionError
    prev_close = klines[-2]["close"] if n >= 2 else 0
    if today["close"] > 0 and today["open"] > 0 and prev_close > 0:
    ...
    if S08_MACD_GOLDEN_CROSS and len(klines) >= 26:
        ...
        if macd_result:
            ...
            if golden and golden.get("has_cross"):
                ...
                reasons.append(f"MACD未出现近{S08_MACD_CROSS_DAYS}日金叉")
        else:
...
__all__ = ["check_s08_volume_breakout"]
