...
import config
from strategies.base import make_result
...
def _yang_volume_ratio(klines: list, start: int, end: int) -> float:
    ...
    return round(ya / ia, 2) if ia > 0 else (999 if ya > 0 else 0)
...
def check_s01_red_fat_green_thin(klines: list, fundamental: dict = None, money_flow: dict = None) -> dict:
    ...
    信号分级：强(阳比>=70%且量比>=2.0) / 中(满足条件) / 弱(仅满足一项)
    """
    ...
    p2 = vr >= config.S01_VOL_RATIO_MIN
    passed = p1 and p2
...
def check_s03_pullback_shrink(klines: list, fundamental: dict = None, money_flow: dict = None) -> dict:
    ...
    if vol_ok and ratio < config.S03_VOL_RATIO_MAX * 0.5:
        signal = "强"; score = 0.9
    elif passed:
        signal = "中"; score = 0.7
    elif vol_ok:
