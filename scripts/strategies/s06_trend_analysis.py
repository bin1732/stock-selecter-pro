...
from indicators.technical import calc_ma, calc_adx, check_ma_alignment
...
def check_s06_trend_analysis(
    ...
    # ── 条件一：均线多头排列 ──
    ma_values = {}
    for period in S09_MA_PERIODS:
        ma = calc_ma(klines, period)
        ma_values[f"MA{period}"] = round(ma[-1], 2) if ma and len(ma) > 0 else None

    # 检查多头排列：MA5 > MA10 > MA20 > MA60
    alignment = check_ma_alignment(klines)
...
__all__ = ["check_s06_trend_analysis"]
