...
数据源：东方财富公开估值API（push2.eastmoney.com），fundamental.py 中的 fetch_valuation。
"""
...
# 若不存在对应配置项，使用硬编码默认值
DIVIDEND_YIELD_MIN = getattr(config, "S06_DIVIDEND_YIELD_MIN", 3.0)   # 股息率最低要求(%)
...
def check_s03_high_dividend(
    ...
    if div_yield is not None and div_yield > 0:
        ...
        details["dividend_yield_threshold"] = DIVIDEND_YIELD_MIN
        if div_yield >= DIVIDEND_YIELD_MIN:
            div_pass = True
            reasons.append(f"股息率 {div_yield:.2f}% >= {DIVIDEND_YIELD_MIN}%")
        else:
            reasons.append(f"股息率 {div_yield:.2f}% < {DIVIDEND_YIELD_MIN}% 不达标")
    else:
...
__all__ = ["check_s03_high_dividend"]
