...
代理算法：
- 近20日换手率均值 < 1.5%（筹码锁定；港股/美股无公开换手率时该条件降级跳过）
- 近20日振幅 < 15%（筹码稳定）
...
S11_TURNOVER_MIN_DAYS = getattr(config, "S11_TURNOVER_MIN_DAYS", 10)  # 20日中有效换手率数据的最低天数门槛
...
def check_s11_chip_concentration(
    klines: list[dict],
    ...
    recent_20 = klines[-20:]
    # 换手率数据完整度门槛：20日中至少 S11_TURNOVER_MIN_DAYS 日有真实数据才视为"可用"。
    # 换手率均值按整 20 日窗口口径计算，缺失日计 0；数据完整度不足时判定为不可用。
    valid_turnovers = [k.get("turnover") for k in recent_20 if k.get("turnover")]
    if len(valid_turnovers) >= S11_TURNOVER_MIN_DAYS:
        turnover_available = True
        avg_turnover = sum(valid_turnovers) / len(recent_20)
        details["avg_turnover_20"] = round(avg_turnover, 2)
        details["turnover_threshold"] = S11_TURNOVER_MAX
        details["turnover_valid_days"] = len(valid_turnovers)
        if avg_turnover < S11_TURNOVER_MAX:
            turnover_pass = True
            reasons.append(f"近20日均换手率 {avg_turnover:.2f}%（{len(valid_turnovers)}日有数据）< {S11_TURNOVER_MAX}% 筹码锁定")
        else:
            reasons.append(f"近20日均换手率 {avg_turnover:.2f}%（{len(valid_turnovers)}日有数据）>= {S11_TURNOVER_MAX}% 换手偏高")
    else:
        details["avg_turnover_20"] = None
        details["turnover_valid_days"] = len(valid_turnovers)
        if valid_turnovers:
            details["turnover_note"] = f"换手率数据不足（20日中仅{len(valid_turnovers)}日有数据，需≥{S11_TURNOVER_MIN_DAYS}日），该条件降级跳过"
        else:
            details["turnover_note"] = "换手率数据缺失（港股/美股无公开换手率），该条件降级跳过"
    ...
    if turnover_available:
        # 换手数据可用（A股）：换手 + 振幅为核心条件
        passed = turnover_pass and amp_pass
    else:
        # 港股/美股无换手率数据：降级为振幅 + 均线为核心条件（如实声明，不伪造换手）
        passed = amp_pass and ma_pass
...
__all__ = ["check_s11_chip_concentration"]
