...
def check_s07_bollinger(
    ...
    if len(volumes) >= S10_VOL_LOW_DAYS:
        recent_5_vols = volumes[-S10_VOL_LOW_DAYS:]
        min_5_vol = min(recent_5_vols)
        details["volume"] = int(volumes[-1])
        details["min_5_volume"] = int(min_5_vol)
        details["current_volume"] = int(volumes[-1])
        if volumes[-1] <= min_5_vol * S10_VOL_MIN_TOLERANCE:  # 近N日最低或接近
            vol_shrink_pass = True
            reasons.append("成交量缩至极低水平")
        else:
