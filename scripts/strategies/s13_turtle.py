...
判定逻辑（经典海龟趋势跟踪规则，适配单股筛选场景）：
- 核心通过条件：唐奇安通道突破——收盘价突破近20日最高价（入场通道，不含当日）
- 辅助过滤加分（仅影响评分与详情，不参与通过判定）：
  - 顺势过滤：收盘价在60日均线上方（海龟为顺势策略）
...
def check_s16_turtle(
    klines: list[dict],
    fundamental: Optional[dict] = None,
    money_flow: Optional[dict] = None,
) -> dict:
    """S16 海龟交易策略判定（唐奇安通道突破 + 趋势过滤）。
    ...
    if len(highs) > S16_ENTRY_PERIOD:
        ...
        exit_low = min(lows[-S16_EXIT_PERIOD - 1:-1])  # 不含当日的近N日最低（标准海龟规则）
